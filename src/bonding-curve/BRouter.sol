...
contract BRouter is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
        ...
        returns (uint256, uint256)
    {
        ...
        IBPair(pair).transferTo(to, amountOut);

        IBPair(pair).swap(0, amountOut, amount, 0);
    ...
    function launch(address tokenAddress) public onlyRole(EXECUTOR_ROLE) nonReentrant {
        ...
        address pair = factory.getPair(tokenAddress, assetToken);
        uint256 assetBalance = IBPair(pair).assetBalance();
        IBPair(pair).transferAsset(msg.sender, assetBalance);
    }
        ...
        nonReentrant
    {
        ...
        IBPair(pair).approval(spender, asset, amount);
    }
}
