...
contract AgentToken is ContextUpgradeable, IAgentToken, Ownable2StepUpgradeable {
    ...
    uint256 internal constant basisPointDenominator = 10000;
    uint256 internal constant precision = 100000000000;
    uint256 internal constant maxSwapThresholdMultiplier = 20;

    address public pair;
    bool internal tokenIncludesTaxes;
    IUniswapV2Router02 internal uniswapRouter;

    uint32 public fundedTimestamp;
    uint16 public buyTaxBasisPoints;
    uint16 public sellTaxBasisPoints;
    uint16 public swapThresholdBasisPoints;
    address public pairToken; // The token used to trade for this token

    /**
     * @dev {autoSwapOngoing} We start with {autoSwapOngoing} ON, as we don't want to
     ...
     */
    bool private autoSwapOngoing;

    address public taxReceiver;
    uint128 public taxAmountPendingSwap;
    ...
    function initialize(
        ...
        autoSwapOngoing = true; // We don't want to tax initial liquidity
    }
    ...
    /**
     * @dev function {_computeSupplyParameters}
     *
     * Process provided supply params
     *
     * @param erc20SupplyParameters_ The supply params
     */
    function _computeSupplyParameters(ERC20SupplyParameters memory erc20SupplyParameters_) internal {
        if (erc20SupplyParameters_.maxSupply != erc20SupplyParameters_.lpSupply + erc20SupplyParameters_.vaultSupply) {
    ...
    /**
     * @dev function {_computeTaxParameters}
     *
     * Process provided tax params
     *
     * @param erc20TaxParameters_ The tax params
     */
    function _computeTaxParameters(ERC20TaxParameters memory erc20TaxParameters_)
        internal
        returns (bool tokenHasTax_)
    {
        /**
         * @dev If this
        ...
        if (erc20TaxParameters_.projectBuyTaxBasisPoints == 0 && erc20TaxParameters_.projectSellTaxBasisPoints == 0) {
            ...
            buyTaxBasisPoints = uint16(erc20TaxParameters_.projectBuyTaxBasisPoints);
            sellTaxBasisPoints = uint16(erc20TaxParameters_.projectSellTaxBasisPoints);
            return true;
        }
    }
    ...
    function _createPair() internal returns (address) {
        pair = IUniswapV2Factory(uniswapRouter.factory()).getPair(address(this), pairToken);

        if (pair == address(0)) {
            pair = IUniswapV2Factory(uniswapRouter.factory()).createPair(address(this), pairToken);

            emit LiquidityPoolCreated(pair);
        }

        _liquidityPools.add(pair);

        return (pair);
    }

    /**
     * @dev function {setupInitialLiquidity}
     *
     * Add initial liquidity to the uniswap pair
     *
     * @param lpOwner The recipient of LP tokens
     */
    function setupInitialLiquidity(address lpOwner) external onlyOwnerOrFactory {
        _setupInitialLiquidity(lpOwner);
    }

    /**
     * @dev function {_setupInitialLiquidity}
     *
     * Add initial liquidity to the uniswap pair (internal function that does processing)
     *
     * * @param lpOwner The recipient of LP tokens
     */
    function _setupInitialLiquidity(address lpOwner) internal {
        // Funded date is the date of first funding. We can only add initial liquidity once. If this date is set,
        // we cannot proceed
        if (fundedTimestamp != 0) {
            revert InitialLiquidityAlreadyAdded();
        }

        fundedTimestamp = uint32(block.timestamp);
        ...
        (uint256 amountA, uint256 amountB, uint256 amountLp) = uniswapRouter.addLiquidity(
            address(this),
            pairToken,
        ...
        emit InitialLiquidityAdded(amountA, amountB, amountLp);

        // We now set this to false so that future transactions can be eligibile for autoswaps
        autoSwapOngoing = false;

        IERC20(pair).transfer(lpOwner, amountLp);
    }
    ...
    function isLiquidityPool(address queryAddress_) public view returns (bool) {
         ...
         */
        return (queryAddress_ == pair || _liquidityPools.contains(queryAddress_));
    }
    ...
    /**
     * @dev function {setTaxReceiver} onlyOwnerOrFactory
     *
     * Allows the manager to set the project tax recipient address
     *
     * @param taxReceiver_ New recipient address
     */
    function setTaxReceiver(address taxReceiver_) external onlyOwnerOrFactory {
        taxReceiver = taxReceiver_;
        emit TaxReceiverUpdated(taxReceiver_);
    }
     ...
     *
     * @param newbuyTaxBasisPoints_ The new buy tax rate
     * @param newsellTaxBasisPoints_ The new sell tax rate
     */
    function setProjectTaxRates(uint16 newbuyTaxBasisPoints_, uint16 newsellTaxBasisPoints_)
        external
        onlyOwnerOrFactory
    {
        uint16 oldBuyTaxBasisPoints = buyTaxBasisPoints;
        uint16 oldSellTaxBasisPoints = sellTaxBasisPoints;

        buyTaxBasisPoints = newbuyTaxBasisPoints_;
        sellTaxBasisPoints = newsellTaxBasisPoints_;

        emit ProjectTaxBasisPointsChanged(
            oldBuyTaxBasisPoints, newbuyTaxBasisPoints_, oldSellTaxBasisPoints, newsellTaxBasisPoints_
        );
    }
    ...
    function _transfer(address from, address to, uint256 amount, bool applyTax) internal virtual {
        ...
        uint256 fromBalance = _validatePreTaxTransfer(from, to, amount);

        // Perform autoswap if eligible
        _autoSwap(from, to);

        // Process taxes
        uint256 amountMinusTax = _computeAndApplyTax(applyTax, to, from, amount);
    ...
    /**
     * @dev function {_validatePreTaxTransfer}
     *
     ...
     */
    function _validatePreTaxTransfer(address from_, address to_, uint256 amount_)
        internal
        view
        returns (uint256 fromBalance_)
    {
        // This can't be a transfer to the pool before the funding date
        ...
        if (to_ == pair && from_ != address(this) && fundedTimestamp == 0) {
    ...
    /**
     * @dev function {_computeAndApplyTax}
     *
     ...
     */
    function _computeAndApplyTax(bool applyTax_, address to_, address from_, uint256 sentAmount_)
        internal
        returns (uint256 amountLessTax_)
    {
        amountLessTax_ = sentAmount_;
        unchecked {
            if (tokenIncludesTaxes && applyTax_ && !autoSwapOngoing) {
                uint256 tax;

                // on sell
                if (isLiquidityPool(to_) && totalSellTaxBasisPoints() > 0) {
                    if (sellTaxBasisPoints > 0) {
                        uint256 projectTax = ((sentAmount_ * sellTaxBasisPoints) / basisPointDenominator);
                        taxAmountPendingSwap += uint128(projectTax);
                        tax += projectTax;
                    }
                }
                // on buy
                    ...
                    if (buyTaxBasisPoints > 0) {
                        uint256 projectTax = ((sentAmount_ * buyTaxBasisPoints) / basisPointDenominator);
                        taxAmountPendingSwap += uint128(projectTax);
                        tax += projectTax;
                    }
                }
    ...
    function _autoSwap(address from_, address to_) internal {
        if (tokenIncludesTaxes) {
            uint256 contractBalance = balanceOf(address(this));
            uint256 swapBalance = contractBalance;

            uint256 swapThresholdInTokens = (_totalSupply * swapThresholdBasisPoints) / basisPointDenominator;

            if (_canExecuteSwap(from_, to_, swapBalance, swapThresholdInTokens)) {
                // Store that a swap back is in progress:
                autoSwapOngoing = true;
                // Check if we need to reduce the amount of tokens for this swap:
                if (swapBalance > swapThresholdInTokens * maxSwapThresholdMultiplier) {
                    swapBalance = swapThresholdInTokens * maxSwapThresholdMultiplier;
                }
     ...
     */
    function _canExecuteSwap(address from_, address to_, uint256 taxBalance_, uint256 swapThresholdInTokens_)
        internal
        view
        returns (bool)
    {
        return (
            taxBalance_ >= swapThresholdInTokens_ && !autoSwapOngoing && !isLiquidityPool(from_)
                && from_ != address(uniswapRouter) && to_ != address(uniswapRouter)
        );
    }
    ...
    function _swapTax(uint256 swapBalance_, uint256 contractBalance_) internal {
        ...
        try uniswapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            swapBalance_, 0, path, taxReceiver, block.timestamp + 600
        ) {
            ...
            if (swapBalance_ < contractBalance_) {
                taxAmountPendingSwap -= uint128((taxAmountPendingSwap * swapBalance_) / contractBalance_);
            } else {
                taxAmountPendingSwap = 0;
            }
        } catch {
     ...
     * if the tax balance is above maxSwapThresholdMultiplier.
     *
    ...
    function distributeTaxTokens() external {
        if (taxAmountPendingSwap > 0) {
            uint256 projectDistribution = taxAmountPendingSwap;
            taxAmountPendingSwap = 0;
            _transfer(address(this), taxReceiver, projectDistribution, false);
        }
    }
